...
bool GeoAc_EstimateEigenray(double Source_Loc[3], double Receiver_Loc[2], double theta_min, double theta_max, double & theta_estimate, double & phi_estimate, double & theta_next, int bounces, double azimuth_error_limit, GeoAc_Sources_Struct &sources, SplineStruct &splines){
    ...
    while(fabs(d_phi) > azimuth_error_limit && iterations < 5){
        double GeoAc_phi =     (90.0 - phi)*Pi/180.0;
        r = GC_r_rcvr;
        r_prev = GC_r_rcvr;
